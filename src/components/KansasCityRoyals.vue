<template>
  <div class="page-container">
    ...
    <img src="https://ftw.usatoday.com/wp-content/uploads/sites/90/2014/10/kansas_city_royals.jpg?w=1000&h=600&crop=1" alt="Logo" />
  </div>
</template>
...
export default defineComponent({
  ...
  const fetchRoster = async () => {
    try {
      const response = await fetch('https://statsapi.mlb.com/api/v1/teams/118/roster'); // Example API endpoint
      if (!response.ok) {
  ...
  const fetchInfo = async () => {
    try {
      const response = await fetch('https://statsapi.mlb.com/api/v1/teams/118'); // Example API endpoint
      if (!response.ok) {
        throw new Error('Failed to fetch posts');
      }
      const data = await response.json();
