<template>
  <div class="page-container">
    ...
    <img src="https://images.squarespace-cdn.com/content/v1/5ff494b9931735466ce28d5b/1611176073756-TE81N0WNFF6YY5UFIWXS/New+York+Yankees+Logo.png?format=1000w" alt="Logo" />
  </div>
</template>
...
export default defineComponent({
  ...
  const fetchRoster = async () => {
    try {
      const response = await fetch('https://statsapi.mlb.com/api/v1/teams/147/roster'); // Example API endpoint
      if (!response.ok) {
  ...
  const fetchInfo = async () => {
    try {
      const response = await fetch('https://statsapi.mlb.com/api/v1/teams/147'); // Example API endpoint
      if (!response.ok) {
        throw new Error('Failed to fetch posts');
      }
      const data = await response.json();
