  <template>
    <div class="page-container">
      ...
      <img src="https://img.mlbstatic.com/mlb-images/image/private/t_16x9/t_w1536/mlb/wxhy57hm1hx4rfnpmavl.jpg" alt="Logo" />
    </div>
  </template>
  ...
  export default defineComponent({
    ...
    const fetchRoster = async () => {
      try {
        const response = await fetch('https://statsapi.mlb.com/api/v1/teams/109/roster'); // Example API endpoint
        if (!response.ok) {
    ...
    const fetchInfo = async () => {
      try {
        const response = await fetch('https://statsapi.mlb.com/api/v1/teams/109'); // Example API endpoint
        if (!response.ok) {
          throw new Error('Failed to fetch posts');
        }
        const data = await response.json();
