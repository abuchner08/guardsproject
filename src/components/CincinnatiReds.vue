<template>
  <div class="page-container">
    ...
    <img src="https://target.scene7.com/is/image/Target/GUEST_259a5010-0428-4d7b-a864-8239de54e77d?wid=1200&hei=1200&qlt=80&fmt=webp" alt="Logo" />
  </div>
</template>
...
export default defineComponent({
  ...
  const fetchRoster = async () => {
    try {
      const response = await fetch('https://statsapi.mlb.com/api/v1/teams/113/roster'); // Example API endpoint
      if (!response.ok) {
  ...
  const fetchInfo = async () => {
    try {
      const response = await fetch('https://statsapi.mlb.com/api/v1/teams/113'); // Example API endpoint
      if (!response.ok) {
        throw new Error('Failed to fetch posts');
      }
      const data = await response.json();
