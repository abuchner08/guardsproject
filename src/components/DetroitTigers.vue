<template>
  <div class="page-container">
    ...
    <img src="https://cdn.statically.io/img/i2.wp.com/www.emilysportnews.com/wp-content/uploads/2023/12/detroit-tigers-logo-9wqnu0hspa35ztkl.jpg" alt="Logo" />
  </div>
</template>
...
export default defineComponent({
  ...
  const fetchRoster = async () => {
    try {
      const response = await fetch('https://statsapi.mlb.com/api/v1/teams/116/roster'); // Example API endpoint
      if (!response.ok) {
  ...
  const fetchInfo = async () => {
    try {
      const response = await fetch('https://statsapi.mlb.com/api/v1/teams/116'); // Example API endpoint
      if (!response.ok) {
        throw new Error('Failed to fetch posts');
      }
      const data = await response.json();
