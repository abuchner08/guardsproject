<template>
  <div class="page-container">
    ...
    <img src="https://imageio.forbes.com/i-forbesimg/media/lists/teams/minnesota-twins_416x416.jpg?format=jpg" alt="Logo" />
  </div>
</template>
...
export default defineComponent({
  ...
  const fetchRoster = async () => {
    try {
      const response = await fetch('https://statsapi.mlb.com/api/v1/teams/142/roster'); // Example API endpoint
      if (!response.ok) {
  ...
  const fetchInfo = async () => {
    try {
      const response = await fetch('https://statsapi.mlb.com/api/v1/teams/142'); // Example API endpoint
      if (!response.ok) {
        throw new Error('Failed to fetch posts');
      }
      const data = await response.json();
