<template>
  <div class="page-container">
    ...
    <img src="https://target.scene7.com/is/image/Target/GUEST_9a0aca91-93a7-447a-807b-57f49789488f?wid=1200&hei=1200&qlt=80&fmt=webp" alt="Logo" />
  </div>
</template>
...
export default defineComponent({
  ...
  const fetchRoster = async () => {
    try {
      const response = await fetch('https://statsapi.mlb.com/api/v1/teams/108/roster'); // Example API endpoint
      if (!response.ok) {
  ...
  const fetchInfo = async () => {
    try {
      const response = await fetch('https://statsapi.mlb.com/api/v1/teams/108'); // Example API endpoint
      if (!response.ok) {
        throw new Error('Failed to fetch posts');
      }
      const data = await response.json();
