<template>
  <div class="page-container">
    ...
    <img src="https://i.ebayimg.com/images/g/JXMAAOSw~xVgV3sK/s-l1600.webp" alt="Logo" />
  </div>
</template>
...
export default defineComponent({
  ...
  const fetchRoster = async () => {
    try {
      const response = await fetch('https://statsapi.mlb.com/api/v1/teams/135/roster'); // Example API endpoint
      if (!response.ok) {
  ...
  const fetchInfo = async () => {
    try {
      const response = await fetch('https://statsapi.mlb.com/api/v1/teams/135'); // Example API endpoint
      if (!response.ok) {
        throw new Error('Failed to fetch posts');
      }
      const data = await response.json();
